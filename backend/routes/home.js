...
import buildStore, { apiAction } from '../../frontend/src/redux';
...
router.get('/*', (req,res) => {
	...
	/**
	 * check path to decide if it needs to fetch additional api data
	 */
	let toBeResolve = [];
	if(req.url == '/about'){
		toBeResolve.push(
	...
	/**
	 * resovle data dependency if any and render the page
	 */
	Promise.all(toBeResolve)
		.then( () => {
